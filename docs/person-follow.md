...
- **연결**: 카메라(이름 콤보, 프리셋), 백엔드(`sim` / `sitl` / `auto` / COMx). `SITL ARM` 은 sitl 백엔드에서만 활성.
- **모드**: `1 STANDBY`(스틱 중립·스로틀 최소) → `2 HOVER`(호버 트림 유지) → `3 FOLLOW`(사람 추적).
  `PANIC`(Space/Esc) 은 즉시 STANDBY. 상태 줄에 현재 스로틀·트림·학습 오프셋·µs·하강량이 표시된다.
...
## 호버가 위로 날아가는 문제 — 원인과 대책

증상: HOVER 에서도 FOLLOW 에서도 기체가 강하게 상승한다.

원인:

1. **HOVER 는 개루프다.** 고도 센서가 없어서 `hover_throttle` 한 값을 그대로 보낸다. 처음 기본값 -0.2(≈40%, 1398µs)는
   Gazebo 모델 기준이었고, 실물 1S 훕은 보통 30~35% 에서 호버하므로 계속 상승했다. 기본값을 **-0.35(≈33%)** 로 낮췄다.
...
5. 이상 시 **SH 놓기** 가 1순위 (즉시 조종기 스틱 복귀). PANIC 은 스로틀 최소이므로 공중에서는 추락한다 (offset 모드는 예외).
