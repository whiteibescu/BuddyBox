...
> ⚠️ **실전 이슈 (2026-07-18 디버깅 결과)**: 나사조임식 플러그는 단자 라벨이
> 실제 접점과 다른 경우가 흔합니다. 본 프로젝트의 플러그는 **GND를 ⏚ 단자가
> 아니라 "R" 단자에 연결해야** 동작했습니다 (⏚ 단자는 내부 미연결 추정).
> 증상: 신호는 인식되다 끊기다 반복 + **금속(스틱 등)을 만질 때만 안정 연결**
> (= 몸이 임시 접지가 되는 현상). 이 증상이 보이면 GND를 다른 단자로
> 옮겨가며 시도해볼 것.
